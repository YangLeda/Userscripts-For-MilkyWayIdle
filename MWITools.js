    ...
    async function handleItemTooltipWithEnhancementLevel(tooltip) {
        ...
        if (best) {
            ...
            appendHTMLStr = `<div style="color: ${SCRIPT_COLOR_TOOLTIP};"><div>${
                isZH
                    ? `强化模拟（强化等级${input_data.enhancing_level}，房子等级${input_data.laboratory_level}，强化器加成${input_data.enhancer_bonus}% ，手套加成${input_data.glove_bonus}%${input_data.tea_enhancing ? '，强化茶' : ''}${input_data.tea_super_enhancing ? '，超级强化茶' : ''}${input_data.tea_ultra_enhancing ? '，究极强化茶' : ''}${input_data.tea_blessed ? '，幸运茶' : ''}，卖单价收货，工时费${numberFormatter(input_data.time_fee)}/小时)：`
                    : `Enhancement simulator: level ${input_data.enhancing_level} enhancing, level ${input_data.laboratory_level} house, ${input_data.enhancer_bonus}% enhancer bonus, ${input_data.glove_bonus}% gloves bonus${input_data.tea_enhancing ? ', enhancing tea' : ''}${input_data.tea_super_enhancing ? ', super enhancing tea' : ''}${input_data.tea_ultra_enhancing ? ', ultra enhancing tea' : ''}${input_data.tea_blessed ? ', blessed tea' : ''}, sell order price in, ${numberFormatter(input_data.time_fee)} hourly fee`
