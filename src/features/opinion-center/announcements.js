  ...
  Object.freeze({
    ...
    body: Object.freeze({
      zh: Object.freeze([
        ...
        "任务筛选按钮现在可用鼠标或触屏长按 1 秒分别锁定；命中锁定类型的任务会让两种刷新选项变灰并显示锁图标。筛选中刷新出的卡片即使改变类型也会保留到重新进入任务页，并会在刷新确认完成后立即更新任务图片；即使游戏只更新任务标题的文字节点，图片也会自动同步，不再需要点击返回。关闭任务统计筛选栏会清除已有锁定。锁定只显示小锁，不会改变筛选高亮，再次长按解锁后小锁会立即消失。长按进度会在按住 0.5 秒后才开始显示，满 1 秒才切换锁定；普通短按会立即切换并高亮筛选，刷新确认层打开时锁图标和筛选高亮也会即时同步。",
      ]),
      en: Object.freeze([
        ...
        "Task filter buttons can now be locked individually with a one-second mouse or touch hold. Tasks matching a locked type gray out both reroll choices and show lock icons. Cards rerolled while filtering remain visible even if their type changes until the task page is re-entered, and their artwork updates immediately once the reroll is confirmed. Artwork now also synchronizes when the game updates only the task title text, without requiring Back navigation. Disabling task statistics filters clears existing locks. Locking only shows the small padlock without changing filter highlighting, and another one-second hold unlocks it and removes the padlock immediately. Hold progress now appears only after a 0.5-second grace period and toggles the lock at one second; ordinary taps immediately toggle and highlight filters, while lock icons and filter highlighting stay synchronized even when the reroll confirmation is open.",
      ]),
    }),
  }),
  Object.freeze({
